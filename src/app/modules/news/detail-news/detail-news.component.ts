  ...
  save() {
    ...
    if(this.data.action == 'add' ){
      this.newsService.createNews(payload).subscribe( 
        resAdd => {
          this.messageService.clearAll();
          this.messageService.showMessage( 'success', 'Thêm mới', 'Thêm mới tin tức thành công' );
          this.dialogRef.close({updated: true});
          this.spinner.hide();
        },
        errUpdate => {
          this.messageService.showMessage( 'error', 'Thêm mới', `Thêm mới tin tức không thành công.  <div> ${errUpdate?.error?.message} </div>` );
          this.spinner.hide();
        }
      )
    } else if( this.data.action == 'edit' ){
      this.newsService.updateNews( this.data.news.id , payload).subscribe( 
        resUpdate => {
          this.messageService.clearAll();
          this.messageService.showMessage( 'success', 'Chỉnh sửa', 'Cập nhật tin tức thành công' );
          this.dialogRef.close({updated: true});
          this.spinner.hide();
        },
        errUpdate => {
          this.messageService.showMessage( 'error', 'Chỉnh sửa', `Cập nhật tin tức không thành công.  <div> ${errUpdate?.error?.message} </div>` );
          this.spinner.hide();
        }
      )
    }
