...
export class ListRoomsComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: TableColumnModel = {
    ...
    'info': { title: 'Tên loại phòng', key: 'info', minWidth: '200px', width: '220px' },
  ...
  removeRoom(news: News) {
    ...
    const confirmConfig: ConfirmDialogConfig = {
      icon: 'error_outline',
      iconColor: 'danger',
      mainTitle: 'Xóa loại phòng',
      message: 'Bạn có muốn xóa loại phòng đang chọn không?',
      actionButtons: [
    ...
    confirmDialog.afterClosed().subscribe(result => {
      if (result.data === true) {
        this.spinner.show();
        this.roomService.removeRoom(news?.id).subscribe(
          resRemove => {
            this.messageService.clearAll();
            this.messageService.showMessage('success', 'Loại phòng', 'Xóa thành công');
            this.spinner.hide();
            this.getLstRooms();
          },
          resRemove => {
            this.messageService.clearAll();
            this.messageService.showMessage('error', 'Loại phòng', 'Xóa không thành công');
            this.spinner.hide();
          }
        )
      }
  ...
  detailRoom(action: 'add' | 'edit' | 'view' , news?: News ) {
    const configDialog = this.dialog.open(DetailRoomComponent, {
      height: '100vh',
      minWidth: '900px',
      maxWidth: '900px',
