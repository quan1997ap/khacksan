...
export class DetailRoomComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DetailRoomComponent>,
  ...
  save() {
    ...
    if(this.data.action == 'add' ){
      this.roomService.createRoom(payload).subscribe( 
        resAdd => {
          this.messageService.clearAll();
          this.messageService.showMessage( 'success', 'Thêm mới', 'Thêm mới loại phòng thành công' );
          this.dialogRef.close({updated: true});
          this.spinner.hide();
        },
        errUpdate => {
          this.messageService.showMessage( 'error', 'Thêm mới', `Thêm mới loại phòng không thành công.  <div> ${errUpdate?.error?.message} </div>` );
          this.spinner.hide();
        }
      )
    } else if( this.data.action == 'edit' ){
      this.roomService.updateRoom( this.data.news.id , payload).subscribe( 
        resUpdate => {
          this.messageService.clearAll();
          this.messageService.showMessage( 'success', 'Chỉnh sửa', 'Cập nhật loại phòng thành công' );
          this.dialogRef.close({updated: true});
          this.spinner.hide();
        },
        errUpdate => {
          this.messageService.showMessage( 'error', 'Chỉnh sửa', `Cập nhật loại phòng không thành công.  <div> ${errUpdate?.error?.message} </div>` );
          this.spinner.hide();
        }
      )
    }
